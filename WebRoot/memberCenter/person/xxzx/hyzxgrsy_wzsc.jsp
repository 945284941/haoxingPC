	...
	function _delete(id){
			if(id==undefined){
				...
				if(window.confirm("确定删除?")){
					$.get('newsCenter/news!deleteWzsc.action',{"ids":ids},function(data){
						window.location.href="xxzx/toWzsc.html?page="+$('#page').val();
					});
				};
			}else{
				ids=id;
...
<body>
	<s:action name="common!toHead" executeResult="true" namespace="/"></s:action>
		<s:action name="common!toLogo" executeResult="true" namespace="/"></s:action>
		   ...
		   <div class="breadThumb">	首页 &gt; 会员中心&gt; 新闻资讯&gt;文章收藏</div>
	<div class="gzgz">
		<div class="hyleft">
			<c:choose>
		...
		<div class="hyright">
		    ...
		    <div class="myToolbar">
		    	...
		    	<div class="fn-right">
		    	<form id="searchForm" name="searchForm" action="xxzx/toWzsc.html"  method="post">
			...
			<table class="table table-bordered myTableList">
				...
				<tbody>
					<c:forEach items="${mcNews }" var="mn">
					<tr>
						<td><input type="checkbox" value="${mn.COLLECT_ID }" name="check" /></td>
						<td><a href="news/detail/${mn.ID }.html" target="_blank" title="${mn.FIRST_TITLE }">${fn:substring(mn.FIRST_TITLE , 0, 20)}</a></td>
						<td class="text-center">健康资讯</td>
						<td class="text-center"><fmt:formatDate value="${mn.COLLECT_TIME }" pattern="yyyy/MM/dd" /></td>
						<td class="text-center opt"><a href="news/detail/${mn.ID }.html" target="_blank">查看</a><a href="javascript:_delete('${mn.COLLECT_ID}')">移除</a></td>
					</tr>
					</c:forEach>
					<tr>
						<td colspan="5">
						<page:pagination path="xxzx/toWzsc.html" formName="searchForm" />
						</td>
					</tr>
				</tbody>
			</table>
