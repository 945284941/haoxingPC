...
<script type="text/javascript">
	var tip = {
		...
		start : function(obj, name, num, tel) {
			var self = this;
			var t = self.$("mjs:tip");
			obj.onmousemove = function(e) {
				var mouse = self.mousePos(e);
				t.innerHTML = "<span>&nbsp&nbsp&nbsp&nbsp请去相关物流网站查看物流信息！</br>物流公司：" + name + "</br>物流单号：" + num
						+ "</br>物流电话：" + tel + "</span>";
					layer.tips(t.innerHTML, this, {
				    guide: 2,
				    style: ['background-color:#cf1529; color:#fff', '#cf1529'],
				    maxWidth:240,
				    closeBtn:[0, true],
				    time: 15
					});
	...
	function receiving(orderId){
		$.ajax( {
						url : '/person/order/receivingOrders.html',
						type : 'POST',
						data : 'id=' + orderId,
						success : function(data) {
							var r = $.parseJSON(data);
							if (r == 'ok') {
								alert("感谢您的支持，您现在可以对商品进行评价！");
								window.location.href = "person/order/myChOrders/0.html";
							} else{
								alert("操作失败！");
								return false;
							}
...
<body>
	...
	<!-- 页脚开始 -->
	<div class="dht">首页 > 会员中心 > 电子商城 > 订单管理 > 串货购买订单</div>
	<div class="gzgz">
		<div class="hyleft">
			<c:choose>
		...
		<div class="hyright">
			<div class="hyrightr">
				<div id="rightjxw">
					...
					<p class="hyd0">
						未付款未发货订单数<span>${orderStatisticsInfo.noPayAndShipOrderCount}</span>
					</p>
					<p class="hyd0">
						已付款未发货订单数<span>${orderStatisticsInfo.alreadyPayAndNoShipOrderCount}</span>
					</p>
					<p class="hyd0">
						已付款已发货订单数<span>${orderStatisticsInfo.alreadyPayAndShipOrderCount}</span>
					</p>
					<p class="hyd0">
						历史交易完成订单数<span>${orderStatisticsInfo.completeOfTheTransactionHistoryOrderCount}</span>
					</p>
					<p class="hyd0">
						历史交易关闭订单数<span>${orderStatisticsInfo.closeOfTheTransactionHistoryOrderCount}</span>
					</p>
					<p class="hyd0">
						已退货未退款订单数<span>${orderStatisticsInfo.alreadyReturnAndNoFullrefundOrderCount}</span>
					</p>
					<p class="hyd0">
						已退货已退款订单数<span>${orderStatisticsInfo.alreadyReturnAndFullrefundOrderCount}</span>
					</p>
			...
			<p class="hymainbt">
				<span class="grmenubt">我的购买订单</span>
			</p>
			<div class="aqwt aqwtdd">
				<form action="/person/order/myChOrders/0.html"
					...
					<ul>
						...
						<li><button type="submit" class="ddqd">确认</button></li>
					</ul>
				</form>
			...
			<div class="lbcontant2">
				<div class="lbctitle lbctitledd">
					<span style="width:173px; border-left:0">商品名称</span><span
						...
						style="width:45px;">评价</span><span style="width:127px;">操作</span>
				</div>
				<s:iterator var="o" value="orderList" status="status">
				
					<div class="sclb2">

						<span style="width:130px;">编号：${o.orderNum}</span><span
							style="width:180px;">下单时间：<s:date name="#o.createtime"
								format="yyyy-MM-dd HH:mm:ss" />
						</span> 
						<span style="width:120px;">商家：<a class="dsjxx"
							href="Shop/index_${o.companyId}.html">${o.companyName}</a> </span>
							<span
							style="width:120px;">还款日：<s:date name="#o.hkTime"
								format="yyyy-MM-dd" />
						</span> 
						...
						<div class="zxlb zxlb2 wddd">
							<ul>
								...
								<li class="lbone" style="height:32px;"><a target="_Blank"
									href="goods/${i.goodsId}.html">${i.goodsName}</a>
								</li>
								...
								<s:if test="#st.index ==0">
									...
									<li class="ddsix"><span class="s1"
										style=" display: block;width:90px; height:18px; text-align: center;">
											<s:if test="#o.payStatus ==0">
												<a href="lh/orderToPay/${o.id}.html">未付款</a>
											</s:if> <s:elseif test="#o.payStatus ==1">已付款</s:elseif> <s:elseif
												test="#o.payStatus ==2">待退款</s:elseif> <s:elseif
												test="#o.payStatus ==3">已退款</s:elseif> <s:elseif
												test="#o.payStatus ==4">已拒绝退款</s:elseif> </span> <span
										style=" display: block;width:90px; height:18px; text-align: center;">
											<s:if test="#o.shipStatus ==0">未发货</s:if> <s:elseif
												...
												test="#o.shipStatus ==4">已退货</s:elseif> </span> <span
										style=" display: block;width:90px; height:18px; text-align: center;">
											<s:if test="#o.shipStatus ==1">
												<a target="_blank"
													onmouseover="tip.start(this,'${o.logisticsName}','${o.logisticsNum}','${o.logisticsTel}')"
													>查看物流</a>
											</s:if> </span></li>
								</s:if>
								<s:else>
								...
								<s:if test="#st.index ==0">
									<li class="ddeight" style="height:32px; line-height:32px;">
										<s:if test="#o.payStatus ==1 && #o.shipStatus==1">
											<!-- <form method="post"
												action="http://i.chinaecpay.com/account_deal_a2m_receive.html"
												target="_blank">
												<input type="hidden" name="dealId" value="${o.dealId}" /> <input
													type="submit" class="ddsh" value="收货" />
											</form> -->
											<a onclick="receiving('${o.id}');"><input class="ddsh" value="收货" /></a>
										</s:if> <s:else>
											<span class="ddsh">收货</span>
										</s:else> 
										
										<!--
										<s:if
											test="#o.payStatus ==1 && (#o.shipStatus ==0 || #o.shipStatus ==1)">
											  <a class="tk"
												onclick="returnGoodsOnclick('${o.id}','${o.shipStatus}','${o.totalCost}');">退货/退款</a>
												<a class="tk"
												onclick="returnGoodsOnclick('${o.id}','${o.orderNum}','${o.shipStatus}','${o.totalCost}');">
										...
										-->
										<s:if test="#o.hkType == 0">
										<a class="tk" href="lh/orderToPayLjhk/${o.id}.html">立即还款</a>
										</s:if>
										<s:if test="#o.hkType == 1">
											<span class="wtk ddsh">已还款</span>
										</s:if>
										</li>
								</s:if>
								<s:else>
				...
				<!-- 分页开始 -->
				<div id="showpages">
					<page:pagination path="person/order/myChOrders/0.html"
	...
	<div id="shxx" class="aqmmtc" style=" visibility:hidden">
		<form action="person/order/receiveGoods.html" id="receiveGoodsFrom"
			...
			<ul>
				...
				<li><input type="password" name="payPassword" id="payPassword"
					value="" /> <input type="hidden" name="order.id" id="payOrderId"
					value="" /></li>
