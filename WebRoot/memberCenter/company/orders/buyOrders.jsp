		...
		<div class="hyright">
			<div class="hyrightr">
				<div id="rightjxw">
					...
					<p class="hyd0">未付款未发货订单数<span>${orderStatisticsInfo.noPayAndShipOrderCount}</span></p>
            		<p class="hyd0">已付款未发货订单数<span>${orderStatisticsInfo.alreadyPayAndNoShipOrderCount}</span></p>
            		<p class="hyd0">已付款已发货订单数<span>${orderStatisticsInfo.alreadyPayAndShipOrderCount}</span></p>
           			<p class="hyd0">历史交易完成订单数<span>${orderStatisticsInfo.completeOfTheTransactionHistoryOrderCount}</span></p>
		            <p class="hyd0">历史交易关闭订单数<span>${orderStatisticsInfo.closeOfTheTransactionHistoryOrderCount}</span></p>
		            <p class="hyd0">已退货未退款订单数<span>${orderStatisticsInfo.alreadyReturnAndNoFullrefundOrderCount}</span></p>
		            <p class="hyd0">已退货已退款订单数<span>${orderStatisticsInfo.alreadyReturnAndFullrefundOrderCount}</span></p>
			...
			<div class="aqwt aqwtdd">
				<form action="/person/order/myOrders.html" id="person_orders_orderForm" name="person_orders_orderForm" method="post">
					<ul>
			...
			<div class="lbcontant2">
				...
				<s:iterator var="o" value="orderList" status="status">
					<div class="sclb2">
						...
						<span style="width:200px;">订单编号：${o.id}</span><span style="width:180px;">下单时间：<s:date name="#o.createtime" format="yyyy-MM-dd HH:mm:ss"/></span>
						<span style="width:190px;">商家信息：<a class="dsjxx" href="Shop/index_${o.companyId}.html">${o.companyName}</a> </span>
					</div>
					<s:iterator var="i" value="#o.items" status="st">
						<div class="zxlb zxlb2 wddd">
							<ul>
								<li class="lbtwo"><a target="_Blank" href="goods/${i.goodsId}.html"><img src="/images/memberimg/sc01.gif" /></a></li>
								<li class="lbone"><a target="_Blank" href="goods/${i.goodsId}.html">${i.goodsName}</a></li>
								...
								<s:if test="#st.index ==0">
									...
									<li class="ddsix">
										...
										<span>
											...
											<s:elseif test="#o.shipStatus ==3">待退货</s:elseif>
										...
										<s:if test="#o.payStatus ==1">
											...
											<s:if test="#o.shipStatus ==2">
												<s:if test="#i.appraise ==0">
													<a onclick="memberEvaluateOnclick('${i.orderId}','${i.goodsId}','${i.companyId}');">未评价</a>
												</s:if>
								...
								<s:if test="#st.index ==0">
									<li class="ddeight">
										<s:if test="#o.payStatus ==1 && #o.shipStatus==1">
											<a onclick="memberPayOrder('${o.id}');">收货</a>							
										</s:if>
										<s:else>
											<span>收货</span>
										</s:else>
										<s:if test="#o.payStatus ==1 && (#o.shipStatus ==0 || #o.shipStatus ==1)">
											<a class="tk" onclick="showsplbtitle('tanchu', 'thtk');">退货/退款</a>
										</s:if>
										<s:else>
											<span  class="wtk">退货/退款</span>
										</s:else>
				...
				<!-- 分页开始 -->
				<div id="showpages">
					<page:pagination path="person/order/myOrders.html" formName="person_orders_orderForm" />
				</div>
				<!-- 分页结束 -->
			</div>
		</div>
