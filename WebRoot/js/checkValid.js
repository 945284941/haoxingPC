/**
 * version:1.0
 * description:可以通过JSON格式进行表单有效性规则配置，基本过程如下所示：
 *     var checker = new CheckValid(config);//创建一个检验器
 *      var result =c.checkAll();//根据config规则校验表单
 *     config的格式参见sample.html
 *
 *实例1:
 *    var config = {
 ...
 validResult.setMsg(prop.label+"只能为偶数");
    ...
    init:function()
    {
        ...
        for (var j = 0; j < props.length; j++)
        {
            ...
            if (this.reportMode == 'hint')
            {
                ...
                var propElem = getPropElem(propName);
        ...
        for (propName in this.validResultMap) {
            ...
            if (validResult.status == -1)
            {
                if (this.reportMode == 'alert')
                {
                    ...
                    alert(errorMsg);
                    try {
                        getPropElem(validResult.propName).select();
                    } catch(e) {
                    }
                    return false;
                }
