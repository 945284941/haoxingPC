		...
		<div class="subcontent">
			...
			<p class="gd-p">
				<span>联系人：${obj.linkman }</span><span>联系电话：<strong
					style="color:#d90000">${obj.linkmobile }</strong> </span><span class="yxtime">有效期至：<fmt:formatDate
						value="${obj.effectivetime }" pattern="yyyy/MM/dd" />
				</span>
			</p>
			<p>
				<span title="${obj.name }">设备名称：${fn:substring(obj.name, 0, 9)}</span><span>价格：${obj.price }元</span><span>型号：${obj.equipmentType
					}</span><span>成色：${obj.quality }</span>
			</p>
			<p>
				<span style="width:auto; margin-right:30px;">所在地区：${obj.address
					}</span><span>QQ/Email：${obj.qqEmail }</span>
			</p>
		</div>
		<div class="subfb">
