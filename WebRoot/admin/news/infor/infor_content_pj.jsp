...
<body id="mainbody" onload="slideShow()">
	...
	<div class="gzgz_content">
		...
		<div class="subcontent">
			<h2>
				<span class="gybt" title="${obj.title}">${fn:substring(obj.title, 0, 14)}</span><span class="gytime">发布时间：<fmt:formatDate
						value="${obj.createtime }" pattern="yyyy-MM-dd" /> </span>
			</h2>
			<p class="gd-p">
				<span>联系人：${obj.linkman }</span><span>联系电话：<strong
					style="color:#d90000">${obj.linkmobile }</strong> </span><span
					class="yxtime">有效期至：<fmt:formatDate
						value="${obj.effectivetime }" pattern="yyyy/MM/dd" /> </span>
			</p>
			<p>
				<span title="${obj.name }">配件名称：${fn:substring(obj.name, 0, 9)}</span><span title="${obj.carBn }">原厂编号：${fn:substring(obj.carBn, 0, 9)}</span><span>生产厂家：${obj.producerName
					}</span><span title="${obj.carcatName }">配件分类：${fn:substring(obj.carcatName,0,10)}</span>
			</p>
			<p>
				<span style="width:460px; margin-right:80px">所在地区：${obj.address
					}</span><span>价格：${obj.price }元</span>
			</p>
			<p>
	...
	<div class="gzgz_content">
		<div class="gzgz_coleft">
			...
			<div class="sm">
				<h3>配件信息：</h3>
				<p>
					<span style=" display:inline-block;width:380px; margin-right:30px;">适用品牌：${obj.carbrandName
						}</span><span>适用车系：${obj.carseriesName }</span>
				</p>
				<p>
					<span style="width:720px;">适用车型：${obj.cartypeName }</span>
				</p>
			</div>
			<div class="sm">
				<h3>配件说明：</h3>
