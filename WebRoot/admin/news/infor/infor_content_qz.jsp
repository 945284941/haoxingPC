...
<body id="mainbody" onload="slideShow()">
	...
	<div class="gzgz_content">
		<div class="subcontent subcontent2">
			<h2>
				<span class="gybt">${obj.title }</span><span class="gytime"><fmt:formatDate
						value="${obj.createtime }" pattern="yyyy-MM-dd" /> </span>
				<div style=" clear:both"></div>
			</h2>
			<p class="gd-p">
				<span>联系人：${obj.linkman }</span><span>联系电话：${obj.linkmobile }</strong>
				</span><span class="yxtime">有效期至：<fmt:formatDate
						value="${obj.effectivetime }" pattern="yyyy/MM/dd" /> </span>
			</p>
			<p class="zpxx">
				<span>期望职位：${obj.job }</span><span>期望月薪：${obj.monthlypay }元</span><span>期望地点：${obj.jobPlace
	...
	<div class="gzgz_content">
		<div class="gzgz_coleft">
			...
			<div class="grxx">
				<h3>个人信息：</h3>
				<p>
					<span>性质：${obj.jobNature }</span><span>性别：<c:choose>
							<c:when test="${obj.gender =='1'}">男</c:when>
							<c:otherwise>女</c:otherwise>
						</c:choose>
					</span><span>出生日期：<fmt:formatDate
						value="${obj.birthday }" pattern="yyyy-MM-dd" /> </span><span>工作经验：${obj.workExperience
						}年</span><span>最高学历：${obj.education }</span><span>证书：${obj.certificate
						}</span><span>现居住地：${obj.homeAddress}</span><span>籍贯：${obj.originPlace
						}</span><span>期望职位：${obj.job }</span><span>期望月薪：${obj.monthlypay
						}元</span><span>期望地点：${obj.jobPlace }</span><span class="zpxx4">QQ/Email：${obj.qqEmail
						}</span>
				</p>
			</div>
			<div class="grxx">
			...
			<div class="hzsj">
				<ul>
					<li><div class="hzsj-img">
						...
						<div class="hzsj-jj">简介：济南蓝海汽车配件有限公司成立于2004年，注册资金200万……</div>
					</li>
					<li><div class="hzsj-img">
							<a href="#"><img src="/images/hzsj.gif" /> </a>
						</div>
