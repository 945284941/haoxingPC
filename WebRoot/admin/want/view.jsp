...
<div class="main">
    ...
    <div class="lh_fbqg_xq">
        <div class="lh_fbqg_xq_left">
            ...
            <p>求购分类：<span>${wantBuy.catName}</span></p>
            <p>求购国家：<span>${wantBuy.countryName}</span></p>
            </c:if>
            <c:if test="${'zh' ne sessionInfo.language}">
                <p>求购分类：<span>${wantBuy.catEnName}</span></p>
                <p>求购国家：<span>${wantBuy.countryNameEng}</span></p>
            </c:if>
            <p>所在地址：<span>${wantBuy.fromAddress}</span></p>
            <p>联系电话：<span>${wantBuy.mobile}</span></p>
            <p>联系邮箱：<span>${wantBuy.email}</span></p>
            <p>求购内容：<span>${wantBuy.content}</span></p>
